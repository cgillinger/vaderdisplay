 ...
 * + FAS 3: ANIMATED ICONS INTEGRATION - amCharts SVG med intelligent fallback
 ...
 * + SVG ICONS: Intelligent hantering av amCharts SVG-ikoner med Weather Icons fallback
 */
...
let dashboardState = {
    ...
    // FAS 3: Animated Icons state
    animatedIconsEnabled: false,    // Läses från API config
    svgLoaderAvailable: false,      // Kontrollerar om SVG-loader laddats
    svgLoaderInitialized: false     // Spårar om SVG-loader initierats
};
...
// === FAS 3: ANIMATED ICONS SYSTEM ===

/**
 * Kontrollera om SVG-loader är tillgänglig och kan användas
 */
function checkSVGLoaderAvailability() {
    // Kontrollera om SVGIconLoader finns som global variabel
    const svgLoaderExists = typeof window.SVGIconLoader !== 'undefined';
    const initFunctionExists = typeof window.initializeSVGLoader === 'function';
    const loadFunctionExists = typeof window.loadAnimatedIcon === 'function';
    
    const available = svgLoaderExists && initFunctionExists && loadFunctionExists;
    
    dashboardState.svgLoaderAvailable = available;
    
    console.log(`🎬 FAS 3: SVG Loader tillgänglighet:`, {
        svgLoaderExists,
        initFunctionExists, 
        loadFunctionExists,
        available
...
/**
 * Initiera SVG-loader om den är tillgänglig och aktiverad
 */
async function initializeSVGLoader() {
    try {
        // Kontrollera om animated icons är aktiverat
        if (!dashboardState.animatedIconsEnabled) {
            console.log('🎬 FAS 3: Animated Icons inaktiverat - hoppar över SVG-loader init');
            return false;
        }
        
        // Kontrollera tillgänglighet
        if (!checkSVGLoaderAvailability()) {
            console.warn('⚠️ FAS 3: SVG Icon Loader ej tillgänglig - saknas svg-icon-loader.js?');
            return false;
        }
        
        // Initiera SVG-loader
        console.log('🎬 FAS 3: Initialiserar SVG Icon Loader...');
        const success = await window.initializeSVGLoader();
        
        if (success) {
            dashboardState.svgLoaderInitialized = true;
            console.log('✅ FAS 3: SVG Icon Loader initialiserad');
            return true;
        } else {
            console.warn('⚠️ FAS 3: SVG Icon Loader initialisering misslyckades');
            return false;
        }
        
    } catch (error) {
        console.error('❌ FAS 3: SVG Icon Loader initialisering fel:', error);
        return false;
    }
}

/**
 * Uppdatera animated icons konfiguration från API-respons
 */
function updateAnimatedIconsConfig(apiData) {
    if (apiData && apiData.config) {
        const animatedIconsEnabled = apiData.config.animated_icons_enabled || false;
        
        if (animatedIconsEnabled !== dashboardState.animatedIconsEnabled) {
            dashboardState.animatedIconsEnabled = animatedIconsEnabled;
            console.log(`🎬 FAS 3: Animated Icons config uppdaterad: ${animatedIconsEnabled}`);
            
            // Initiera SVG-loader om det aktiverades
            if (animatedIconsEnabled && !dashboardState.svgLoaderInitialized) {
                initializeSVGLoader().catch(error => {
                    console.warn('SVG-loader init misslyckades:', error);
                });
            }
        }
    }
...
/**
 * Intelligent huvudväderikonuppdatering med SVG-stöd
 */
async function updateMainWeatherIcon(weatherSymbol, isDay = true) {
    const iconElement = document.getElementById('smhi-weather-icon');
    if (!iconElement) {
        console.warn('Huvudväderikon-element saknas');
        return;
    }
    
    let iconUpdated = false;
    
    // FAS 3: Försök med SVG först om tillgängligt
    if (dashboardState.animatedIconsEnabled && dashboardState.svgLoaderInitialized) {
        try {
            console.log(`🎬 FAS 3: Försöker ladda SVG för symbol ${weatherSymbol} (dag: ${isDay})`);
            
            const svgResult = await window.loadAnimatedIcon(weatherSymbol, isDay);
            
            if (svgResult && svgResult.success && svgResult.element) {
                // Rensa befintligt innehåll
                iconElement.innerHTML = '';
                iconElement.className = 'weather-icon';
                
                // Lägg till SVG-element
                svgResult.element.classList.add('weather-main-icon');
                iconElement.appendChild(svgResult.element);
                
                console.log(`✅ FAS 3: SVG-ikon laddad: ${svgResult.source} (${svgResult.iconPath})`);
                iconUpdated = true;
            } else {
                console.log(`⚠️ FAS 3: SVG misslyckades för symbol ${weatherSymbol}: ${svgResult?.error || 'Okänt fel'}`);
            }
            
        } catch (error) {
            console.warn('🔄 FAS 3: SVG-laddning fel, faller tillbaka till Weather Icons:', error);
        }
    }
    
    // Fallback till Weather Icons om SVG misslyckades eller inte är tillgängligt
    if (!iconUpdated) {
        ...
        console.log(`🔄 FAS 3: Weather Icons fallback: ${iconName} för symbol ${weatherSymbol}`);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Weather Dashboard FAS 3: Graciös UI-degradering aktiverad med HUMIDITY FIX + Animated Icons');
    initializeDashboard();
    startDataUpdates();
    startThemeCheck();
});
...
async function initializeDashboard() {
    try {
        console.log('📊 FAS 3: Initialiserar graciös dashboard med HUMIDITY FIX + Animated Icons...');
        ...
        // FAS 3: SVG Loader initialization - efter första data-load
        if (dashboardState.animatedIconsEnabled) {
            initializeSVGLoader().catch(error => {
                console.warn("SVG Loader initialization failed:", error);
            });
        }
        
        console.log('✅ FAS 3: Graciös Dashboard med HUMIDITY FIX + Animated Icons initialiserat!');
    } catch (error) {
...
async function updateAllData() {
    try {
        ...
        // FAS 3: Uppdatera Animated Icons config
        updateAnimatedIconsConfig(currentData);
...
// === UI UPDATE FUNCTIONS (UPPDATERAD FÖR FAS 3 + HUMIDITY FIX + ANIMATED ICONS) ===
...
function updateCurrentWeather(data) {
    ...
    if (data.smhi) {
        ...
        // FAS 3: SMHI Väder-ikon med intelligent SVG/Weather Icons hantering
        if (smhi.weather_symbol) {
            const isDay = isDaytime();
            
            // Använd intelligent ikon-uppdatering
            updateMainWeatherIcon(smhi.weather_symbol, isDay);
...
console.log('✅ FAS 3: Weather Dashboard med Graciös UI-degradering + HUMIDITY FIX + Animated Icons laddat! 💧🎨🚀🎬');